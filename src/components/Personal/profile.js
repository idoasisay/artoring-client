...
const { classNameReplacer } = utils;
// 최상위에서 관리하는 프로파일과 여기서 관리하는 프로파일일 겹친다.
// 따로 페이지를 개발때의 어쩔수 없이 여기서 상태관리를 진행했다.
// 최상된에서 관리되는 프로필을 사용해야 한다.

const Profile = (props) => {
  // count는 profile 상태 내부에서 관리될 관심사 상태를 주입하기 위한 일종의 플래그
  ...
  // count 값이 0 일때만 profile에 관심사 리스트를 profile에 추가한다. 최상단 props로 통폐합 이후에는 사용하지 않게될것
  useEffect(() => {
  ...
  // 컴포넌트 마운트 이후? 암튼 입력을 하지 않더라도 유효성 검증을 시작.
      ...
      {props.profile
        ? <form onSubmit={handleSubmit(onSubmit)}>
          <label>
            ...
            {/*
              * react-hook-form 사용법은 공식문서 를 참고할 것
          ...
          <label>
            <div className='PlaceHolderBtnContainer'>
              <div
                ...
                onMouseOver={(e) => classNameReplacer(e, 'BtnType5', 'BtnType5 Btn5Active')}
                onMouseLeave={(e) => classNameReplacer(e, 'BtnType5 Btn5Active', 'BtnType5')}
              >변경사항 저장
              </div>
            </div>
          </label>
